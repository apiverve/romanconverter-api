...
class RomanconverterAPIClientError(Exception):
...
class ValidationError(RomanconverterAPIClientError):
...
class RomanconverterAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"number": {"type": "integer", "required": True, "min": 1, "max": 3999}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the RomanconverterAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises RomanconverterAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/romanconverter'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise RomanconverterAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[RomanconverterAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns RomanNumeralConverterData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises RomanconverterAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise RomanconverterAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise RomanconverterAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise RomanconverterAPIClientError(f"Request failed: {str(e)}")
